    ...
    ItemTouchHelper.SimpleCallback(0,ItemTouchHelper.LEFT)
{
    ...
    private fun drawButton(c: Canvas, itemView: View, buffer: MutableList<MyButton>, translationX: Float) {
        var right = itemView.right.toFloat()
        val dButtonWidth = -1*translationX/buffer.size
        for (button in buffer){
            val left = right - dButtonWidth
            button.onDraw(c, RectF(left,itemView.top.toFloat(),right,itemView.bottom.toFloat()), pos = 1 )
            right = left
        }
    }
}